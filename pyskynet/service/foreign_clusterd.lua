...
local function open_channel(t, key)
	...
	if address == nil then
		local host, port = string.match(key, "^(%d+[.]%d+[.]%d+[.]%d+):(%d+)$")
		if host then
			skynet.error("set default cluster node [".. key.."]")
			node_address[key] = key
			address = key
		elseif not config.nowaiting then
			local co = coroutine.running()
			...
			skynet.error("unknown cluster node [".. key.."]")
		end
	end
	local succ, err, c
	if address then
...
function command.socket(source, subcmd, fd, msg)
	if subcmd == "open" then
		...
		local agent = skynet.newservice("foreign_clusteragent", skynet.self(), source, fd)
